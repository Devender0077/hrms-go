...
export default function PaymentTypeSettings() {
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/payment-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load payment type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/payment-types/${selectedItem?.id}` : "/hr-setup/payment-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Payment Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save payment type", color: "danger" });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this payment type?")) {
      try {
        const response = await apiRequest(`/hr-setup/payment-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Payment Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete payment type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Payment Type Management</h3>
            <p className="text-sm text-default-500">Manage payment methods and types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:wallet" />} onPress={onOpen}>Add Payment Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Payment Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No payment type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Payment Type" : "Add New Payment Type"}</ModalHeader>
              <ModalBody>
