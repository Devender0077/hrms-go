...
export default function GoalTypeSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<GoalType[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<GoalType | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/goal-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load goal type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/goal-types/${selectedItem?.id}` : "/hr-setup/goal-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Goal Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save goal type", color: "danger" });
    }
  };

  const handleEdit = (item: GoalType) => {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this goal type?")) {
      try {
        const response = await apiRequest(`/hr-setup/goal-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Goal Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete goal type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Goal Type Management</h3>
            <p className="text-sm text-default-500">Manage goal types and categories</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:target" />} onPress={onOpen}>Add Goal Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Goal Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No goal type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Goal Type" : "Add New Goal Type"}</ModalHeader>
              <ModalBody>
