...
export default function PayslipTypeSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<PayslipType[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<PayslipType | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/payslip-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load payslip types", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/payslip-types/${selectedItem?.id}` : "/hr-setup/payslip-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Payslip type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save payslip type", color: "danger" });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this payslip type?")) {
      try {
        const response = await apiRequest(`/hr-setup/payslip-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Payslip type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete payslip type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Payslip Type Management</h3>
            <p className="text-sm text-default-500">Manage payslip types and templates</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:plus" />} onPress={onOpen}>Add Payslip Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Payslip types">
            <TableHeader>
            ...
            <TableBody emptyContent="No payslip types found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Payslip Type" : "Add New Payslip Type"}</ModalHeader>
              <ModalBody>
