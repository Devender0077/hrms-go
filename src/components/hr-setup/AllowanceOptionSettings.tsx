...
export default function AllowanceOptionSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<AllowanceOption[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<AllowanceOption | null>(null);
  const [formData, setFormData] = useState({ name: "", code: "", description: "", is_taxable: true, status: "active" as const });
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/allowance-options");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load allowance options", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/allowance-options/${selectedItem?.id}` : "/hr-setup/allowance-options";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Allowance option ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save allowance option", color: "danger" });
    }
  };

  const handleEdit = (item: AllowanceOption) => {
    setSelectedItem(item);
    setFormData({ name: item.name, code: item.code || "", description: item.description || "", is_taxable: item.is_taxable, status: item.status as 'active' });
    setIsEditing(true);
    onOpen();
  };

  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this allowance option?")) {
      try {
        const response = await apiRequest(`/hr-setup/allowance-options/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Allowance option deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete allowance option", color: "danger" });
      }
    }
  };

  const handleCloseModal = () => {
    setFormData({ name: "", code: "", description: "", is_taxable: true, status: "active" });
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Allowance Option Management</h3>
            <p className="text-sm text-default-500">Manage employee allowance types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:plus" />} onPress={onOpen}>Add Allowance</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Allowance options">
            <TableHeader>
            ...
            <TableBody emptyContent="No allowance options found">
              {items.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell><Chip color={item.is_taxable ? 'warning' : 'default'} variant="flat" size="sm">{item.is_taxable ? 'Taxable' : 'Non-taxable'}</Chip></TableCell>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Allowance Option" : "Add New Allowance Option"}</ModalHeader>
              <ModalBody>
                <div className="space-y-4">
                  ...
                  <Switch isSelected={formData.is_taxable} onValueChange={(value) => setFormData({ ...formData, is_taxable: value })}>
                    <div>
                      ...
                      <p className="text-sm text-default-500">Subject to income tax</p>
                    </div>
                  </Switch>
                </div>
              </ModalBody>
