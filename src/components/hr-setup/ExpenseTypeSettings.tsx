...
export default function ExpenseTypeSettings() {
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/expense-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load expense type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/expense-types/${selectedItem?.id}` : "/hr-setup/expense-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Expense Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save expense type", color: "danger" });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this expense type?")) {
      try {
        const response = await apiRequest(`/hr-setup/expense-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Expense Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete expense type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Expense Type Management</h3>
            <p className="text-sm text-default-500">Manage expense categories and types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:shopping-bag" />} onPress={onOpen}>Add Expense Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Expense Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No expense type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Expense Type" : "Add New Expense Type"}</ModalHeader>
              <ModalBody>
