...
export default function CompetenciesSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<Competencies[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<Competencies | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/competencies");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load competencies", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/competencies/${selectedItem?.id}` : "/hr-setup/competencies";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Competencies ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save competencies", color: "danger" });
    }
  };

  const handleEdit = (item: Competencies) => {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this competencies?")) {
      try {
        const response = await apiRequest(`/hr-setup/competencies/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Competencies deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete competencies", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Competencies Management</h3>
            <p className="text-sm text-default-500">Manage employee competencies and skills</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:award" />} onPress={onOpen}>Add Competencies</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Competencies table">
            <TableHeader>
            ...
            <TableBody emptyContent="No competencies found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Competencies" : "Add New Competencies"}</ModalHeader>
              <ModalBody>
