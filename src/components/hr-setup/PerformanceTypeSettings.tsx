...
export default function PerformanceTypeSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<PerformanceType[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<PerformanceType | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/performance-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load performance type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/performance-types/${selectedItem?.id}` : "/hr-setup/performance-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Performance Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save performance type", color: "danger" });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this performance type?")) {
      try {
        const response = await apiRequest(`/hr-setup/performance-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Performance Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete performance type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Performance Type Management</h3>
            <p className="text-sm text-default-500">Manage performance review types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:activity" />} onPress={onOpen}>Add Performance Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Performance Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No performance type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Performance Type" : "Add New Performance Type"}</ModalHeader>
              <ModalBody>
