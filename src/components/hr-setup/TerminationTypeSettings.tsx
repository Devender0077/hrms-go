...
export default function TerminationTypeSettings() {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [items, setItems] = useState<TerminationType[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedItem, setSelectedItem] = useState<TerminationType | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/termination-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load termination type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/termination-types/${selectedItem?.id}` : "/hr-setup/termination-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Termination Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save termination type", color: "danger" });
    }
  };

  const handleEdit = (item: TerminationType) => {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this termination type?")) {
      try {
        const response = await apiRequest(`/hr-setup/termination-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Termination Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete termination type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Termination Type Management</h3>
            <p className="text-sm text-default-500">Manage termination reasons and types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:user-x" />} onPress={onOpen}>Add Termination Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Termination Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No termination type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Termination Type" : "Add New Termination Type"}</ModalHeader>
              <ModalBody>
