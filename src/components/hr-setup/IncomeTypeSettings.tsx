...
export default function IncomeTypeSettings() {
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await apiRequest("/hr-setup/income-types");
      if (response.success) setItems(response.data || []);
    } catch (error) {
      addToast({ title: "Error", description: "Failed to load income type", color: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      const url = isEditing ? `/hr-setup/income-types/${selectedItem?.id}` : "/hr-setup/income-types";
      const response = await apiRequest(url, { method: isEditing ? "PUT" : "POST", body: formData });
      if (response.success) {
        addToast({ title: "Success", description: `Income Type ${isEditing ? "updated" : "created"} successfully`, color: "success" });
        loadItems();
        handleCloseModal();
      }
    } catch (error) {
      addToast({ title: "Error", description: "Failed to save income type", color: "danger" });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this income type?")) {
      try {
        const response = await apiRequest(`/hr-setup/income-types/${id}`, { method: "DELETE" });
        if (response.success) {
          addToast({ title: "Success", description: "Income Type deleted successfully", color: "success" });
          loadItems();
        }
      } catch (error) {
        addToast({ title: "Error", description: "Failed to delete income type", color: "danger" });
      }
    }
  };
    ...
    <>
      <Card className="shadow-sm">
        <CardHeader className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold">Income Type Management</h3>
            <p className="text-sm text-default-500">Manage income categories and types</p>
          </div>
          <Button color="primary" startContent={<Icon icon="lucide:arrow-up-right" />} onPress={onOpen}>Add Income Type</Button>
        </CardHeader>
        <CardBody>
          <Table aria-label="Income Type table">
            <TableHeader>
            ...
            <TableBody emptyContent="No income type found">
              {items.map((item) => (
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>{isEditing ? "Edit Income Type" : "Add New Income Type"}</ModalHeader>
              <ModalBody>
