/**
 * Hindi (hi) Translations
 */

export const hi = {
  // Navigation
  'Dashboard': 'डैशबोर्ड',
  'Calendar': 'कैलेंडर',
  'Tasks': 'कार्य',
  'Organization Chart': 'संगठन चार्ट',
  'Employees': 'कर्मचारी',
  'Employee Documents': 'कर्मचारी दस्तावेज़',
  'Employee Salaries': 'कर्मचारी वेतन',
  'Departments': 'विभाग',
  'Designations': 'पदनाम',
  'Branches': 'शाखाएं',
  'Attendance': 'उपस्थिति',
  'Attendance Muster': 'उपस्थिति रजिस्टर',
  'Time Tracking': 'समय ट्रैकिंग',
  'Leave Applications': 'छुट्टी आवेदन',
  'Leave Types': 'छुट्टी के प्रकार',
  'Holidays': 'छुट्टियां',
  'Jobs': 'नौकरियां',
  'Candidates': 'उम्मीदवार',
  'Interviews': 'साक्षात्कार',
  'Payroll Overview': 'वेतन अवलोकन',
  'Salary Slips': 'वेतन पर्ची',
  'Payroll Reports': 'वेतन रिपोर्ट',
  'Training Sessions': 'प्रशिक्षण सत्र',
  'Employee Training': 'कर्मचारी प्रशिक्षण',
  'Goals': 'लक्ष्य',
  'Reviews': 'समीक्षा',
  'Attendance Reports': 'उपस्थिति रिपोर्ट',
  'Leave Reports': 'छुट्टी रिपोर्ट',
  'Users': 'उपयोगकर्ता',
  'Roles & Permissions': 'भूमिकाएं और अनुमतियां',
  'HR System Setup': 'एचआर सिस्टम सेटअप',
  'Settings': 'सेटिंग्स',
  'Messenger': 'मैसेंजर',
  'Announcements': 'घोषणाएं',
  
  // Section Titles
  'Main': 'मुख्य',
  'HR Management': 'एचआर प्रबंधन',
  'Timekeeping': 'समय प्रबंधन',
  'Leave Management': 'छुट्टी प्रबंधन',
  'Recruitment': 'भर्ती',
  'Payroll': 'वेतन',
  'Training': 'प्रशिक्षण',
  'Performance': 'प्रदर्शन',
  'Reports': 'रिपोर्ट',
  'System': 'सिस्टम',
  
  // Common Actions
  'Save': 'सहेजें',
  'Cancel': 'रद्द करें',
  'Edit': 'संपादित करें',
  'Delete': 'हटाएं',
  'Add': 'जोड़ें',
  'Create': 'बनाएं',
  'Update': 'अपडेट करें',
  'View': 'देखें',
  'Search': 'खोजें',
  'Filter': 'फ़िल्टर',
  'Export': 'निर्यात',
  'Import': 'आयात',
  'Submit': 'जमा करें',
  'Close': 'बंद करें',
  'Loading': 'लोड हो रहा है',
  'Success': 'सफलता',
  'Error': 'त्रुटि',
  'Warning': 'चेतावनी',
  'Info': 'जानकारी',
  'Apply': 'लागू करें',
  'Reset': 'रीसेट करें',
  'Clear': 'साफ़ करें',
  'Download': 'डाउनलोड',
  'Upload': 'अपलोड',
  'Print': 'प्रिंट',
  'Refresh': 'रीफ्रेश',
  'Back': 'वापस',
  'Next': 'अगला',
  'Previous': 'पिछला',
  'Continue': 'जारी रखें',
  'Finish': 'समाप्त',
  
  // Status
  'Active': 'सक्रिय',
  'Inactive': 'निष्क्रिय',
  'Pending': 'लंबित',
  'Approved': 'स्वीकृत',
  'Rejected': 'अस्वीकृत',
  'Completed': 'पूर्ण',
  'In Progress': 'प्रगति में',
  'Not Started': 'शुरू नहीं हुआ',
  'On Hold': 'होल्ड पर',
  'Cancelled': 'रद्द',
  'Draft': 'ड्राफ्ट',
  'Published': 'प्रकाशित',
  'Enabled': 'सक्षम',
  'Disabled': 'अक्षम',
  'Online': 'ऑनलाइन',
  'Offline': 'ऑफ़लाइन',
  'Present': 'उपस्थित',
  'Absent': 'अनुपस्थित',
  'Late': 'विलंब',
  'On Leave': 'छुट्टी पर',
  'Half Day': 'आधा दिन',
  'Full Day': 'पूर्ण दिन',
  'New': 'नया',
  'Read': 'पढ़ा गया',
  'Unread': 'अपठित',
  
  // Common Labels
  'Name': 'नाम',
  'Email': 'ईमेल',
  'Phone': 'फ़ोन',
  'Address': 'पता',
  'Status': 'स्थिति',
  'Date': 'तारीख',
  'Time': 'समय',
  'Actions': 'क्रियाएं',
  'Description': 'विवरण',
  'Department': 'विभाग',
  'Designation': 'पदनाम',
  'Role': 'भूमिका',
  'Permissions': 'अनुमतियां',
  'First Name': 'पहला नाम',
  'Last Name': 'अंतिम नाम',
  'Full Name': 'पूरा नाम',
  'Phone Number': 'फ़ोन नंबर',
  'Mobile': 'मोबाइल',
  'Employee ID': 'कर्मचारी आईडी',
  'Employee Name': 'कर्मचारी का नाम',
  'Username': 'उपयोगकर्ता नाम',
  'Password': 'पासवर्ड',
  'Confirm Password': 'पासवर्ड की पुष्टि करें',
  'Gender': 'लिंग',
  'Date of Birth': 'जन्म तिथि',
  'Joining Date': 'शामिल होने की तारीख',
  'Created At': 'बनाया गया',
  'Updated At': 'अपडेट किया गया',
  'Last Login': 'अंतिम लॉगिन',
  'Type': 'प्रकार',
  'Category': 'श्रेणी',
  'Priority': 'प्राथमिकता',
  'Title': 'शीर्षक',
  'Content': 'सामग्री',
  'Check In': 'चेक इन',
  'Check Out': 'चेक आउट',
  
  // Form Placeholders
  'Enter your name': 'अपना नाम दर्ज करें',
  'Enter first name': 'पहला नाम दर्ज करें',
  'Enter last name': 'अंतिम नाम दर्ज करें',
  'Enter email': 'ईमेल दर्ज करें',
  'Enter email address': 'ईमेल पता दर्ज करें',
  'Enter phone number': 'फ़ोन नंबर दर्ज करें',
  'Enter password': 'पासवर्ड दर्ज करें',
  'Search...': 'खोजें...',
  'Type to search': 'खोजने के लिए टाइप करें',
  'Confirm your password': 'अपने पासवर्ड की पुष्टि करें',
  'Enter announcement title': 'घोषणा शीर्षक दर्ज करें',
  'Enter announcement content': 'घोषणा सामग्री दर्ज करें',
  
  // Button Labels
  'Add New': 'नया जोड़ें',
  'Create New': 'नया बनाएं',
  'Save Changes': 'परिवर्तन सहेजें',
  'Clear Filters': 'फ़िल्टर साफ़ करें',
  'Select All': 'सभी चुनें',
  'View Details': 'विवरण देखें',
  'View All': 'सभी देखें',
  'Create Announcement': 'घोषणा बनाएं',
  
  // Messages
  'No data found': 'कोई डेटा नहीं मिला',
  'No results found': 'कोई परिणाम नहीं मिला',
  'No announcements found': 'कोई घोषणा नहीं मिली',
  'No users found': 'कोई उपयोगकर्ता नहीं मिला',
  'Loading...': 'लोड हो रहा है...',
  'Loading data...': 'डेटा लोड हो रहा है...',
  'Please wait...': 'कृपया प्रतीक्षा करें...',
  'Success!': 'सफलता!',
  'Error!': 'त्रुटि!',
  'Are you sure?': 'क्या आप सुनिश्चित हैं?',
  'This action cannot be undone': 'यह क्रिया पूर्ववत नहीं की जा सकती',
  'Record saved successfully': 'रिकॉर्ड सफलतापूर्वक सहेजा गया',
  'Record updated successfully': 'रिकॉर्ड सफलतापूर्वक अपडेट किया गया',
  'Record deleted successfully': 'रिकॉर्ड सफलतापूर्वक हटाया गया',
  'Invalid input': 'अमान्य इनपुट',
  'Field is required': 'फ़ील्ड आवश्यक है',
  'Invalid email format': 'अमान्य ईमेल प्रारूप',
  'You have already checked in today': 'आप आज पहले ही चेक इन कर चुके हैं',
  'Are you sure you want to delete this announcement?': 'क्या आप सुनिश्चित हैं कि आप इस घोषणा को हटाना चाहते हैं?',
  'No data available': 'कोई डेटा उपलब्ध नहीं',
  
  // Pagination
  'Showing': 'दिखा रहा है',
  'of': 'का',
  'entries': 'प्रविष्टियां',
  'items': 'आइटम',
  'Page': 'पेज',
  
  // Time & Date
  'Today': 'आज',
  'Yesterday': 'कल',
  'Tomorrow': 'कल',
  'This Week': 'यह सप्ताह',
  'This Month': 'यह महीना',
  'This Year': 'यह वर्ष',
  'Days': 'दिन',
  'Weeks': 'सप्ताह',
  'Months': 'महीने',
  'Years': 'वर्ष',
  'Hours': 'घंटे',
  'Minutes': 'मिनट',
  'From': 'से',
  'To': 'तक',
  
  // Gender
  'Male': 'पुरुष',
  'Female': 'महिला',
  'Other': 'अन्य',
  
  // Priority
  'Low': 'निम्न',
  'Medium': 'मध्यम',
  'High': 'उच्च',
  'Urgent': 'अत्यावश्यक',
  
  // Categories
  'General': 'सामान्य',
  'HR': 'एचआर',
  'Finance': 'वित्त',
  'IT': 'आईटी',
  'Operations': 'संचालन',
  
  // Announcements
  'reads': 'पढ़ा गया',
  'Expires': 'समाप्त',
  'Expires on': 'समाप्ति तिथि',
  'Stay updated with latest company news': 'नवीनतम कंपनी समाचार के साथ अपडेट रहें',
  'View and manage company-wide announcements and important updates': 'कंपनी-व्यापी घोषणाओं और महत्वपूर्ण अपडेट देखें और प्रबंधित करें',
  'All Priorities': 'सभी प्राथमिकताएं',
  'All Categories': 'सभी श्रेणियां',
  'Expires At (Optional)': 'समाप्ति तिथि (वैकल्पिक)',
  
  // Search Related
  'Pages': 'पेज',
  'Main dashboard analytics': 'मुख्य डैशबोर्ड विश्लेषण',
  'Browse all employees': 'सभी कर्मचारियों को ब्राउज़ करें',
  'Track employee attendance': 'कर्मचारी उपस्थिति ट्रैक करें',
  'Manage leave requests': 'छुट्टी अनुरोध प्रबंधित करें',
  'View payroll information': 'वेतन जानकारी देखें',
  'Manage system users': 'सिस्टम उपयोगकर्ता प्रबंधित करें',
  'Manage roles and permissions': 'भूमिकाएं और अनुमतियां प्रबंधित करें',
  'Configure system settings': 'सिस्टम सेटिंग्स कॉन्फ़िगर करें',
  'Company announcements': 'कंपनी घोषणाएं',
  'Send and receive messages': 'संदेश भेजें और प्राप्त करें',
  'Searching...': 'खोज रहे हैं...',
  'Try a different search term': 'एक अलग खोज शब्द आज़माएं',
  'Search for': 'के लिए खोजें',
  'Press Enter to search': 'खोजने के लिए Enter दबाएं',
  
  // Common Phrases
  'Welcome back': 'वापसी पर स्वागत है',
  'Hello': 'नमस्ते',
  'Thank you': 'धन्यवाद',
  'Yes': 'हाँ',
  'No': 'नहीं',
  'Ok': 'ठीक है',
  
  // Users Page
  'Users Management': 'उपयोगकर्ता प्रबंधन',
  'System Access & Permissions': 'सिस्टम एक्सेस और अनुमतियां',
  'Manage system users, roles, and permissions. Control access levels, assign roles, and maintain security across your HRMS platform.': 'सिस्टम उपयोगकर्ताओं, भूमिकाओं और अनुमतियों का प्रबंधन करें। एक्सेस स्तर नियंत्रित करें, भूमिकाएं निर्धारित करें और अपने HRMS प्लेटफ़ॉर्म में सुरक्षा बनाए रखें।',
  'Add User': 'उपयोगकर्ता जोड़ें',
  'Edit User': 'उपयोगकर्ता संपादित करें',
  'User Details': 'उपयोगकर्ता विवरण',
  'Reset Password': 'पासवर्ड रीसेट करें',
  'User Status': 'उपयोगकर्ता स्थिति',
  'Permissions Count': 'अनुमतियों की संख्या',
  'Last Active': 'अंतिम सक्रिय',
  'Never': 'कभी नहीं',
  'Account Active': 'खाता सक्रिय',
  'Account Inactive': 'खाता निष्क्रिय',
  
  // Roles Page
  'Role Management': 'भूमिका प्रबंधन',
  'Access Control': 'एक्सेस नियंत्रण',
  'Manage user roles and their permissions to control access to different parts of the system.': 'सिस्टम के विभिन्न भागों तक पहुंच को नियंत्रित करने के लिए उपयोगकर्ता भूमिकाओं और उनकी अनुमतियों का प्रबंधन करें।',
  'Create Role': 'भूमिका बनाएं',
  'Edit Role': 'भूमिका संपादित करें',
  'Delete Role': 'भूमिका हटाएं',
  'Manage Permissions': 'अनुमतियां प्रबंधित करें',
  'Assign Permissions': 'अनुमतियां निर्धारित करें',
  'No roles found': 'कोई भूमिका नहीं मिली',
  'Role Name': 'भूमिका का नाम',
  'Role Description': 'भूमिका विवरण',
  
  // Dashboard
  'Total Companies': 'कुल कंपनियां',
  'Total Users': 'कुल उपयोगकर्ता',
  'Total Employees': 'कुल कर्मचारी',
  'Active Sessions': 'सक्रिय सत्र',
  '+12% from last month': 'पिछले महीने से +12%',
  '+8% from last month': 'पिछले महीने से +8%',
  '+15% from last month': 'पिछले महीने से +15%',
  'from last month': 'पिछले महीने से',
  'from last week': 'पिछले सप्ताह से',
  'Super Admin Dashboard': 'सुपर एडमिन डैशबोर्ड',
  'Company Admin Dashboard': 'कंपनी एडमिन डैशबोर्ड',
  'Employee Dashboard': 'कर्मचारी डैशबोर्ड',
  
  // Attendance Page
  'Attendance Management': 'उपस्थिति प्रबंधन',
  'Track & Monitor Employee Time': 'कर्मचारी समय ट्रैक और मॉनिटर करें',
  'Track employee attendance, check-in/out times, and work hours. Monitor productivity and ensure accurate time tracking across your organization.': 'कर्मचारी उपस्थिति, चेक-इन/आउट समय और कार्य घंटों को ट्रैक करें। उत्पादकता की निगरानी करें और अपने संगठन में सटीक समय ट्रैकिंग सुनिश्चित करें।',
  'Export Report': 'रिपोर्ट निर्यात करें',
  'Check-in successful': 'चेक-इन सफल',
  'Check-out successful': 'चेक-आउट सफल',
  'No attendance records found': 'कोई उपस्थिति रिकॉर्ड नहीं मिला',
  'Attendance Records': 'उपस्थिति रिकॉर्ड',
  'Check In Time': 'चेक इन समय',
  'Check Out Time': 'चेक आउट समय',
  'Work Hours': 'कार्य घंटे',
  'IP Address': 'आईपी पता',
  
  // Employees Page
  'Employees': 'कर्मचारी',
  'HR Management': 'मानव संसाधन प्रबंधन',
  'Manage your organization\'s workforce with comprehensive employee data, performance tracking, and streamlined HR processes.': 'व्यापक कर्मचारी डेटा, प्रदर्शन ट्रैकिंग और सुव्यवस्थित HR प्रक्रियाओं के साथ अपने संगठन के कार्यबल का प्रबंधन करें।',
  'Add Employee': 'कर्मचारी जोड़ें',
  'Import Employees': 'कर्मचारी आयात करें',
  'Total Employees': 'कुल कर्मचारी',
  'Active Employees': 'सक्रिय कर्मचारी',
  'On Leave': 'छुट्टी पर',
  'Departments': 'विभाग',
  'Search employees...': 'कर्मचारी खोजें...',
  'All Departments': 'सभी विभाग',
  'All Status': 'सभी स्थिति',
  'Active': 'सक्रिय',
  'Inactive': 'निष्क्रिय',
  'Employee Name': 'कर्मचारी का नाम',
  'Employee ID': 'कर्मचारी आईडी',
  'Department': 'विभाग',
  'Designation': 'पदनाम',
  'Email': 'ईमेल',
  'Phone': 'फोन',
  'Status': 'स्थिति',
  'Actions': 'क्रियाएं',
  'View': 'देखें',
  'Edit': 'संपादित करें',
  'Delete': 'हटाएं',
  'Change Password': 'पासवर्ड बदलें',
  'No employees found': 'कोई कर्मचारी नहीं मिला',
  'Loading employees...': 'कर्मचारी लोड हो रहे हैं...',
  
  // Settings Page
  'Settings': 'सेटिंग्स',
  'System Configuration': 'सिस्टम कॉन्फ़िगरेशन',
  'Configure your HRMS system settings, integrations, and preferences.': 'अपने HRMS सिस्टम सेटिंग्स, एकीकरण और प्राथमिकताओं को कॉन्फ़िगर करें।',
  'General Settings': 'सामान्य सेटिंग्स',
  'Company Information': 'कंपनी जानकारी',
  'Localization': 'स्थानीयकरण',
  'Email Configuration': 'ईमेल कॉन्फ़िगरेशन',
  'Notifications': 'सूचनाएं',
  'Integrations': 'एकीकरण',
  'Security & Privacy': 'सुरक्षा और गोपनीयता',
  'Backup & Storage': 'बैकअप और स्टोरेज',
  'API Management': 'API प्रबंधन',
  'Workflow Settings': 'वर्कफ़्लो सेटिंग्स',
  'Reports & Analytics': 'रिपोर्ट और विश्लेषण',
  'Offer Letter Settings': 'ऑफर लेटर सेटिंग्स',
  'Joining Letter Settings': 'जॉइनिंग लेटर सेटिंग्स',
  'Experience Certificate': 'अनुभव प्रमाणपत्र',
  'NOC Settings': 'NOC सेटिंग्स',
  'Google Calendar': 'गूगल कैलेंडर',
  'SEO Settings': 'SEO सेटिंग्स',
  'Cache Settings': 'कैश सेटिंग्स',
  'Webhook Settings': 'वेबहुक सेटिंग्स',
  'Cookie Consent': 'कुकी सहमति',
  'ChatGPT Integration': 'ChatGPT एकीकरण',
  'Export & Import': 'निर्यात और आयात',
  'Basic system configuration': 'बुनियादी सिस्टम कॉन्फ़िगरेशन',
  'Company details and branding': 'कंपनी विवरण और ब्रांडिंग',
  'Language, timezone, and regional settings': 'भाषा, समय क्षेत्र और क्षेत्रीय सेटिंग्स',
  'Email server and notification settings': 'ईमेल सर्वर और सूचना सेटिंग्स',
  'Notification preferences and channels': 'सूचना प्राथमिकताएं और चैनल',
  'Third-party service integrations': 'तृतीय-पक्ष सेवा एकीकरण',
  'Security policies and access control': 'सुरक्षा नीतियां और पहुंच नियंत्रण',
  'Data backup and storage management': 'डेटा बैकअप और स्टोरेज प्रबंधन',
  'API keys and webhook configuration': 'API कुंजी और वेबहुक कॉन्फ़िगरेशन',
  'Approval workflows and automation': 'अनुमोदन वर्कफ़्लो और स्वचालन',
  'Report configuration and analytics': 'रिपोर्ट कॉन्फ़िगरेशन और विश्लेषण',
  'Offer letter templates and automation': 'ऑफर लेटर टेम्प्लेट और स्वचालन',
  'Joining letter templates and configuration': 'जॉइनिंग लेटर टेम्प्लेट और कॉन्फ़िगरेशन',
  'Experience certificate templates and settings': 'अनुभव प्रमाणपत्र टेम्प्लेट और सेटिंग्स',
  'No Objection Certificate templates and settings': 'अनापत्ति प्रमाणपत्र टेम्प्लेट और सेटिंग्स',
  'Google Calendar integration and sync settings': 'गूगल कैलेंडर एकीकरण और सिंक सेटिंग्स',
  'Search engine optimization and meta tags': 'सर्च इंजन अनुकूलन और मेटा टैग',
  'Cache configuration and management': 'कैश कॉन्फ़िगरेशन और प्रबंधन',
  'Webhook endpoints and event configuration': 'वेबहुक एंडपॉइंट और इवेंट कॉन्फ़िगरेशन',
  'Cookie consent banner and privacy settings': 'कुकी सहमति बैनर और गोपनीयता सेटिंग्स',
  'AI assistant and ChatGPT integration settings': 'AI सहायक और ChatGPT एकीकरण सेटिंग्स',
  'Export and import settings configuration': 'निर्यात और आयात सेटिंग्स कॉन्फ़िगरेशन',
  'Save All Changes': 'सभी परिवर्तन सहेजें',
  'Saving...': 'सहेजा जा रहा है...',
  'Loading settings...': 'सेटिंग्स लोड हो रही हैं...',
  'Export Settings': 'सेटिंग्स निर्यात करें',
  'Import Settings': 'सेटिंग्स आयात करें',
  'Reset to Defaults': 'डिफ़ॉल्ट पर रीसेट करें',
  
  // Common Actions
  'Save': 'सहेजें',
  'Cancel': 'रद्द करें',
  'Close': 'बंद करें',
  'Confirm': 'पुष्टि करें',
  'Yes': 'हां',
  'No': 'नहीं',
  'OK': 'ठीक है',
  'Apply': 'लागू करें',
  'Reset': 'रीसेट करें',
  'Clear': 'साफ़ करें',
  'Submit': 'जमा करें',
  'Next': 'अगला',
  'Previous': 'पिछला',
  'Finish': 'समाप्त',
  'Back': 'वापस',
  'Continue': 'जारी रखें',
  'Skip': 'छोड़ें',
  
  // Common Messages
  'Success!': 'सफलता!',
  'Error!': 'त्रुटि!',
  'Warning!': 'चेतावनी!',
  'Info': 'जानकारी',
  'Loading...': 'लोड हो रहा है...',
  'Please wait...': 'कृपया प्रतीक्षा करें...',
  'Are you sure?': 'क्या आप सुनिश्चित हैं?',
  'This action cannot be undone.': 'यह क्रिया पूर्ववत नहीं की जा सकती।',
  'No data available': 'कोई डेटा उपलब्ध नहीं',
  'No results found': 'कोई परिणाम नहीं मिला',
  
  // Tasks Page
  'Tasks': 'कार्य',
  'Task Management': 'कार्य प्रबंधन',
  'Manage and track your tasks efficiently. Stay organized with our comprehensive task management system.': 'अपने कार्यों को कुशलता से प्रबंधित और ट्रैक करें। हमारे व्यापक कार्य प्रबंधन प्रणाली के साथ संगठित रहें।',
  'Add Task': 'कार्य जोड़ें',
  'Total Tasks': 'कुल कार्य',
  'Pending': 'लंबित',
  'In Progress': 'प्रगति में',
  'Completed': 'पूर्ण',
  'Task Title': 'कार्य शीर्षक',
  'Description': 'विवरण',
  'Priority': 'प्राथमिकता',
  'Due Date': 'नियत तिथि',
  'Assignee': 'नियुक्त व्यक्ति',
  'Progress': 'प्रगति',
  'Low': 'कम',
  'Medium': 'मध्यम',
  'High': 'उच्च',
  'Urgent': 'अत्यावश्यक',
  'All Tasks': 'सभी कार्य',
  'My Tasks': 'मेरे कार्य',
  'Assigned by Me': 'मेरे द्वारा सौंपे गए',
  'Overdue': 'समय सीमा समाप्त',
  'No tasks found': 'कोई कार्य नहीं मिला',
  'Create Task': 'कार्य बनाएं',
  'Update Task': 'कार्य अपडेट करें',
  'Task Details': 'कार्य विवरण',
  'Mark as Complete': 'पूर्ण के रूप में चिह्नित करें',
  'Reopen Task': 'कार्य फिर से खोलें',
  
  // Messenger Page
  'Messenger': 'मैसेंजर',
  'Team Communication': 'टीम संचार',
  'Connect with your team members through instant messaging.': 'तत्काल संदेश के माध्यम से अपनी टीम के सदस्यों से जुड़ें।',
  'New Message': 'नया संदेश',
  'Conversations': 'बातचीत',
  'Groups': 'समूह',
  'Online': 'ऑनलाइन',
  'Offline': 'ऑफ़लाइन',
  'Type a message...': 'एक संदेश टाइप करें...',
  'Send': 'भेजें',
  'Search conversations...': 'बातचीत खोजें...',
  'No conversations yet': 'अभी तक कोई बातचीत नहीं',
  'Start a conversation': 'बातचीत शुरू करें',
  'Create Group': 'समूह बनाएं',
  'Group Name': 'समूह का नाम',
  'Add Members': 'सदस्य जोड़ें',
  'Select members...': 'सदस्य चुनें...',
};

