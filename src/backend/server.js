// Express server for HRM API
    ...
    app.get('/api/v1/roles/:role/permissions', authenticateToken, async (req, res) => {
      try {
        ...
        // Map role names from URL format to database format
        const roleNameMap = {
          'super_admin': 'Super Admin',
          'company_admin': 'Super Admin', // Map to Super Admin for now
        ...
        const query = `
          SELECT p.*, rp.created_at as granted_at
          FROM permissions p
    ...
    app.post('/api/v1/leave/applications', async (req, res) => {
      try {
        ...
        const [result] = await pool.query(
          `INSERT INTO leave_applications 
           (application_id, employee_id, leave_type_id, start_date, end_date, total_days,
