...
async function fixDatabase() {
  try {
    ...
    // Test the database connection and show some data
    console.log('\n📊 Testing database...');
    const [employees] = await pool.query('SELECT COUNT(*) as count FROM employees');
    console.log(`✅ Found ${employees[0].count} employees in database`);
    
    const [policies] = await pool.query('SELECT COUNT(*) as count FROM attendance_policies');
    console.log(`✅ Found ${policies[0].count} attendance policies in database`);
